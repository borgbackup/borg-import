...
class rsnapshotImporter(Importer):
    ...
    def populate_parser(self, parser):
        parser.add_argument(
            "--backup-set",
            help="Only consider given backup set (can be given multiple times).",
            action="append",
            dest="backup_sets",
        )
        parser.add_argument(
            "rsnapshot_root", metavar="RSNAPSHOT_ROOT", help="Path to rsnapshot root directory", type=Path
        )
        # TODO: support the full wealth of borg possibilities
        parser.add_argument(
            "repository",
            metavar="BORG_REPOSITORY",
...
class rsynchlImporter(Importer):
    ...
    def populate_parser(self, parser):
        parser.add_argument("rsync_root", metavar="RSYNC_ROOT", help="Path to root directory", type=Path)
        # TODO: support the full wealth of borg possibilities
        parser.add_argument(
            "repository",
            metavar="BORG_REPOSITORY",
...
class borgImporter(Importer):
    ...
    def populate_parser(self, parser):
        ...
        parser.add_argument(
            "repository",
            metavar="DESTINATION_REPOSITORY",
...
def build_parser():
    ...
    common_group.add_argument(
        "--create-options",
        "-o",
        help="Additional borg create options " '(note: Use -o="--foo --bar" syntax to avoid parser confusion).',
    )
    common_group.add_argument("--prefix", help="Add prefix to imported archive names", default="")

    common_group.add_argument(
        "--debug", action="store_const", dest="log_level", const=logging.DEBUG, help="Display debug/trace messages."
    )
    ...
    for importer_class in Importer.__subclasses__():
        ...
        subparser = subparsers.add_parser(
            importer.name,
            help=importer.description,
            epilog=textwrap.dedent(importer.epilog),
